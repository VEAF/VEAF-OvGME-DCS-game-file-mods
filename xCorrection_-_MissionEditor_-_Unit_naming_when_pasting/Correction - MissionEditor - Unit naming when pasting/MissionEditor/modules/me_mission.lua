...
------------------------------------------
--
function getUnitName(a_groupName, a_forbiddenNames, originalUnitName) -- added originalUnitName so it can be passed through to check_unit_name
	local gName = a_groupName.."-1"
	
	return check_unit_name(gName, a_forbiddenNames, originalUnitName) -- passing originalUnitName through to check_unit_name
end
...
function check_unit_name(a_name, a_forbiddenNames, originalUnitName)
    ...
    if magicMarkerIdx then
        -- magic marker found, we'll simply append a numeric value to the unit name
        local num = 1
        local baseName = base.string.sub(originalUnitName, 0,-magicMarkerIdx-magicMarkerLen+2)
        repeat 
          nm = baseName .. base.string.format("%03i", num)
          num = num + 1
        until not(unit_by_name[nm] or (a_forbiddenNames and a_forbiddenNames[nm]))
    else
        -- magic marker not found, let ED's original code work
        local num = 0
        local baseName = nm
