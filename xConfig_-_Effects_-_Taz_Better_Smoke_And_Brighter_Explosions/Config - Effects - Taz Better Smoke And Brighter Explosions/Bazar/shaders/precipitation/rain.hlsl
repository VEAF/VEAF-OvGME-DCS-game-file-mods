...
static const float raindropScale = 0.03;
static const float raindropLengthFactor = 0.15;
static const float raindropBrightness = 8;

static const float rainSpecularPower = 1;
static const float rainSpecularAphaFactor = 0.5;
static const float rainOpacityBase = 0.4;
static const float rainHaloFactor = 2;
// static const float rainHaloAlphaFactor = 0.5333333;
static const float rainHaloAlphaFactor = 0.5;
...
void gsRain(point VS_OUTPUT input[1], inout TriangleStream<PS_INPUT_RAIN> outputStream)
{
	...
	float scale = raindropScale*(0.5+RAND) * (1+pow(nDistToCam*0.6,1.5)*2) * (0.8+0.2*rainPower);//домножаем на масштаб частицы

	float2 rndVel = 1.5 * (noise2(float2(RAND, RAND+3.4344))*2-1) * rainPower * (0.5+0.5*nDistToCam) * (0.5+0.5*sin((gModelTime*2+RAND)*PI));
	
	float3 speedResult = (mul(-cameraVel + rainVel + float3(rndVel.x, 0, rndVel.y), (float3x3)gView))*10;
...
float4 psRain(in PS_INPUT_RAIN i, uniform bool bLighting = false) : SV_TARGET0
{
	...
	if(bLighting)
	{
		float4 additiveLightColor = calculateSumLightingSpecular(toCam, i.wPos.xyz, toCam, rainSpecularPower);
