	...
	uniform bool useShadows, uniform bool useSSAO, uniform bool useSSLR, uniform bool useCockpitGI, uniform bool discardTerrainInsideFog, uniform int mode, uniform bool bSampleFrequency)
{
	...
	case STENCIL_COMPOSITION_MODEL:		return ComposeSample(input, sidx, useShadows, useSSAO, bSampleFrequency, mode, input.stencil & 1, useSSLR);	// input.stencil & 1 ? FAR_ENV_MAP : LERP_ENV_MAP
	case STENCIL_COMPOSITION_COCKPIT:	return ComposeCockpitSample(input, sidx, useShadows, useSSAO, useCockpitGI, bSampleFrequency, mode, useSSLR);
...
{
	if (_HmdMask(i.pos.xy, getBufferSize(StencilMap), MASKSIZE))
    return 0;
  
  return ComposeSampleMain(i, 0, useShadows, useSSAO, useSSLR, useCockpitGI, false, mode, false);
}

bool IsMSAAPixel(float2 pos)
{
  if (_HmdMask(pos, getBufferSize(StencilMap), MSAA_MASKSIZE))
    return false;
...
float4 PS_MSAA_SAMPLE(const VS_COMPOSE_OUTPUT i, uint sidx: SV_SampleIndex, uniform bool useShadows, uniform bool useSSAO, uniform bool useSSLR, uniform bool useCockpitGI, uniform int mode = 0): SV_TARGET0
{
	if (_HmdMask(i.pos.xy, getBufferSize(StencilMap), MASKSIZE))
    return 0;
  if (_HmdMask(i.pos.xy, getBufferSize(StencilMap), MSAA_MASKSIZE))
    return 0;
