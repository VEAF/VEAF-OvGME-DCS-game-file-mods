...
float SampleLuminance(uint2 texelOrig, uniform int downsampling = 1)
{
	float3 color = 0;
	float weight = 1;
  
  // scale sampling so not reading outside masked area
  uint2 texel = (uint2) ((float2)texelOrig * MASKSIZE * 2.0 * 0.707) + (MASKSIZE * 0.707 * 0.293);

#if DOWNSAMPLING > 1 && !ENABLE_SIMPLE_HG
	#if STAGES_COUNT == 1
...
#if ENABLE_SIMPLE_HG
  // simple hack: just sample from middle of cell
  texel = texelOffset + tileSize / 2;
  {
    {
			float lum = SampleLuminance(texel, 1);
#else
	[loop]
	for(texel.x = texelOffset.x; texel.x<texelNext.x; texel.x += DOWNSAMPLING)
	{
		...
		{
			float lum = SampleLuminance(texel, DOWNSAMPLING);
#endif
