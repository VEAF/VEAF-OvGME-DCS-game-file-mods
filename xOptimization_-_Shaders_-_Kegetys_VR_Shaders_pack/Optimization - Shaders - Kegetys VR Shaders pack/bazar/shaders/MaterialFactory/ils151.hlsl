...
float4 psTex(in vsOutput_ils i, uniform bool bTexArray): SV_TARGET0
{
	// added slight mip bias for sharper text
	float bias = -0.66;
	float4 tex = bTexArray? TextureArray.SampleBias(WrapLinearSampler, i.vTexCoord0.xyz, bias) : TextureMap.SampleBias(WrapLinearSampler, i.vTexCoord0.xy, bias);
	//clip(tex.a-0.02);
...
float4 psTexA8(in vsOutput_ils i, uniform bool bTexArray): SV_TARGET0
{
	// added slight mip bias for sharper text
	float bias = -0.66;	
	float tex = bTexArray? TextureArray.SampleBias(WrapLinearSampler, i.vTexCoord0.xyz, bias).a : TextureMap.SampleBias(WrapLinearSampler, i.vTexCoord0.xy, bias).a;
	clip(tex-0.02);
...
float4 psLine(vsOutput_line i, uniform bool bSpecular): SV_TARGET0
{
	...
	if(bSpecular)
	{
		...
		screenSpecular = ShadeSolid(gSunDiffuse, 0, specular, norm, roughness, 0, shadow, 1, viewDir, float2(0, 1));
